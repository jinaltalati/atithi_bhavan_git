                ...
                <table class="table table-hover table-nomargin table-bordered dataTable-custom dataTable-columnfilter dataTable-tools dataTable-colvis myTable">
                   <thead>
                      <tr class="thefilter">
                        ...
                        <th>Rate/Sheet</th>
                        <th>Amount</th>
                        <?php if(false && $this->session->userdata('role_id') == '1') { ?>
                            <th>Job #</th>
                        <?php } ?>
                        <?php } ?>
                      </tr>
                   ...
                   <tbody>
                    ...
                    foreach($rows as $k1 => $v1)
                    {
                        echo '<tr class="header" id="'.$k1.'">';
                            echo '<td width="5%;">'.($k1+1).'</td>';
                            echo '<td width="10%;">'.$v1['trans_date'].'</td>';
                            echo '<td width="10%;">'.$sizes[$v1['size']]->size_w.' X '.$sizes[$v1['size']]->size_h.'</td>';
                            echo '<td width="20%;">'.$papers[$v1['paper']]->paper_name.' ('.$papers[$v1['paper']]->gsm.')'.'</td>';
                            //echo '<td>'.$v1['opening'].'</td>';
                            //echo '<td>'.$v1['sale'].'</td>';
                            //echo '<td>'.$v1['purchase'].'</td>';
                            echo '<td>'.$v1['closing'].'</td>';
                            if ($this->session->userdata('role_id') == "1") {
                            echo '<td>'.$v1['rate_per_sheet'].'</td>';
                            echo '<td>'.($v1['closing'] * $v1['rate_per_sheet']) .'</td>';
                            if(false && $this->session->userdata('role_id') == '1') {
                                echo '<td>'.$v1['job_no'].'</td>';
                            }
                             $sum  = $sum + ($v1['closing'] * $v1['rate_per_sheet']);
                            }
                        echo '</tr>';
                    }
                    ?>
